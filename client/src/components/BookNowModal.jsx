...
const BookNowModal = ({ isOpen, onClose, car, pickupDate, returnDate }) => {
  const { token, axios } = useAppContext(); // ✅ axios + token from context
  ...
  const handleConfirmBooking = async () => {
    if (!pickupDate || !returnDate) {
      return alert("⚠️ Please select pickup and return date first!");
    }

    if (!car?._id) {
      return alert("⚠️ Car details not found!");
    }

    if (!token) {
      return alert("⚠️ Please login first to book a vehicle!");
    }
