...
const Navbar = () => {
  ...
  const openDashboard = () => {
    ...
    if (!key) {
      key = prompt("Enter dashboard key");
      if (!key) return;
  ...
  return (
    ...
    >
      ...
      <div className="hidden sm:flex items-center gap-8">
        ...
        <div className="flex items-center gap-6">
          <button onClick={openDashboard} className="cursor-pointer">
            {isOwner ? "Dashboard" : "List cars"}
          </button>

          <button
            onClick={() => (user ? logout() : setShowLogin(true))}
            className="cursor-pointer px-6 py-2 bg-primary text-white rounded-lg"
          >
            {user ? "Logout" : "Login"}
          </button>
        </div>
